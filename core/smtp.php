...
class SMTP {
    ...
    public function __construct($localhost="",$debug_mode = false)
    {
        // set debug mode
    ...
    public function host($host,$secure="",$port=null)
    {
    	if(!isset($port))
    	{
    ...
    protected function format($recipient)
    {
        ...
        if ($recipient['name'])
        {
            return "=?UTF-8?B?".base64_encode($recipient['name'])."?=".' <'.$recipient['email'].'>';
        }
        else
        {
